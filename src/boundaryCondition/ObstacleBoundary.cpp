...
#pragma acc wait
    ...
    void apply_neumann(real *data_field, size_t *d_patch, Patch p, size_t patch_start, size_t patch_end, size_t level, real value) {
        ...
        apply_boundary_condition(data_field, d_patch, patch_start, patch_end, level, sign_reference_index, reference_index, -value, POSITIVE_SIGN);
    }
    ...
    for (size_t i = 0; i < number_of_patches; i++) {
        ...
        switch (bc) {
            case BoundaryCondition::DIRICHLET:
                apply_dirichlet(data, d_patch, p, patch_start, patch_end, level, boundary_data->get_value(p));
                break;
            case BoundaryCondition::NEUMANN:
                apply_neumann(data, d_patch, p, patch_start, patch_end, level, boundary_data->get_value(p));
                break;
