...
#ifndef BENCHMARKING
    auto logger = Utility::create_logger(settings, "SurfaceBoundary");
...
void apply_dirichlet(Settings::Settings const &settings,
                     Field &field, MultipleJoinedList *mjl,
    ...
    apply_boundary_condition(settings, field, mjl, id, sign_reference_index, reference_index, value * 2, NEGATIVE_SIGN);
}
...
void apply_neumann(Settings::Settings const &settings,
    ...
    apply_boundary_condition(settings, field, mjl, id, sign_reference_index, reference_index, value, POSITIVE_SIGN);
}
...
// *********************************************************************************************
void apply_periodic(Settings::Settings const &settings,
                    Field &field, MultipleJoinedList *mjl,
    ...
    switch (patch) {
        ...
        default:
            ...
            apply_boundary_condition(settings, field, mjl, id, sign_reference_index, reference_index, 0, POSITIVE_SIGN);
        }
    }
    ...
    for (size_t i = 0; i < number_of_patches; i++) {
        ...
        switch (bc) {
            case BoundaryCondition::DIRICHLET:
                ...
                apply_dirichlet(settings, field, mjl, id, p, value);
                break;
            case BoundaryCondition::NEUMANN:
                if (field.get_level() == 0) {
                    value = boundary_data->get_value(p);
                }
                apply_neumann(settings, field, mjl, id, p, value);
                break;
            case BoundaryCondition::PERIODIC:
                apply_periodic(settings, field, mjl, id, p);
                break;
            default:
#ifndef BENCHMARKING
