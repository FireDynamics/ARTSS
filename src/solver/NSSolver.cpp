...
NSSolver2<t_temp, t_turb, t_con>::NSSolver2(FieldController *field_controller) {
#ifndef BENCHMARKING
...
template <bool t_temp, bool t_turb, bool t_con>
NSSolver2<t_temp, t_turb, t_con>::~NSSolver2() {
    delete adv_vel;
    delete dif_vel;
    delete adv_temp;
    delete dif_temp;
...
// ***************************************************************************************
template <bool t_temp, bool t_turb, bool t_con>
void NSSolver2<t_temp, t_turb, t_con>::do_step(real t, bool sync) {
    auto params = Parameters::getInstance();
...
// 1. Solve advection equation
        ...
        if (nu != 0.0) {
            dif_vel->diffuse(u, *u0, *u_tmp, *u_tmp, *v_tmp, *w_tmp, nu, *nu_t, sync);
            dif_vel->diffuse(v, *v0, *v_tmp, *u_tmp, *v_tmp, *w_tmp, nu, *nu_t, sync);
            dif_vel->diffuse(w, *w0, *w_tmp, *u_tmp, *v_tmp, *w_tmp, nu, *nu_t, sync);
        }
...
#endif
                dif_temp->diffuse(T, *T0, *T_tmp, *u_tmp, *v_tmp, *w_tmp, kappa, *kappa_t, sync);
...
#endif
                dif_con->diffuse(C, *C0, *C_tmp, *u_tmp, *v_tmp, *w_tmp, gamma, *gamma_t, sync);
...
// ***************************************************************************************
template <bool t_temp, bool t_turb, bool t_con>
void NSSolver2<t_temp, t_turb,  t_con>::control() {
    auto params = Parameters::getInstance();
#ifndef BENCHMARKING
    auto logger = Utility::create_logger(typeid(NSSolver2).name());
#endif
